...
import {
  MOCK_CUSTOMER_ORDERS, ORDER_STATUS_CONFIG, SERVICE_TYPE_CONFIG,
  SERVICE_STEPS, type OrderData, type OrderStatus
} from "@/data/orderData";
    ...
    return (
      <div className="gradient-surface min-h-screen">
        ...
        <div className="flex flex-col items-center justify-center py-20">
          ...
          <Button variant="outline" className="mt-4" onClick={() => navigate("/customer/orders")}>
            Quay lại
          </Button>
        </div>
      </div>
        ...
        >
          ...
          <div className="flex justify-between items-center">
            <span className="text-[10px] text-muted-foreground">
              {ORDER_STATUS_CONFIG[steps[0]].label}
            </span>
            <span className="text-[10px] font-bold text-primary">
              {currentStepIndex + 1}/{steps.length}
            </span>
            <span className="text-[10px] text-muted-foreground">
              {ORDER_STATUS_CONFIG[steps[steps.length - 1]].label}
            </span>
          </div>
        </motion.div>
              ...
              >
                <div className="mt-3 space-y-0">
                  {[...order.timeline].reverse().map((t, i) => {
                    const isFirst = i === 0;
                    const config = ORDER_STATUS_CONFIG[t.status];
                    return (
                      <div key={i} className="flex gap-3 relative">
                        <div className="flex flex-col items-center">
                          {isFirst ? (
                            ...
                            <Circle size={14} className="text-muted-foreground/40 shrink-0 mt-1" />
                          )}
                        ...
                        <div className="pb-3 min-w-0">
                          <p className={`text-[12px] font-semibold ${isFirst ? "text-foreground" : "text-muted-foreground"}`}>
                            {t.label}
                          </p>
                          <p className="text-[10px] text-muted-foreground/70 mt-0.5">
                            {t.date} {t.actor && `· ${t.actor}`}
                          </p>
                        </div>
                      </div>
                    );
              ...
              <div className="flex items-start gap-2">
                ...
                <div>
                  ...
                  {order.assignedStaff.map((s) => (
                    <p key={s.id} className="text-foreground">• {s.name} ({s.role})</p>
                  ))}
                </div>
              </div>
            )}
      ...
      <Sheet open={showConfirm} onOpenChange={setShowConfirm}>
        <SheetContent side="bottom" className="rounded-t-3xl max-h-[70vh] overflow-y-auto px-5 pb-8">
          <SheetHeader className="pb-4">
            <SheetTitle className="text-base text-left">Xác nhận nghiệm thu</SheetTitle>
          </SheetHeader>
              ...
              <div className="flex justify-center gap-2">
                {[1, 2, 3, 4, 5].map((s) => (
                  <motion.button key={s} onClick={() => setRating(s)} whileTap={{ scale: 0.8, rotate: -15 }} className="p-1">
                    <Star size={32} className={s <= rating ? "text-amber-500 fill-amber-500" : "text-muted-foreground/30"} />
                  </motion.button>
                ))}
              </div>
