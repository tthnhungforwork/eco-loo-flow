...
import {
  ...
  FileText, CheckCircle2, Send, ArrowLeft, ChevronRight,
  Sparkles, Wrench, HardHat, Hammer, Recycle
} from "lucide-react";
    ...
    <div className="gradient-surface min-h-screen">
      ...
      <div className="px-4 py-5">
        ...
        <div className="flex items-center gap-2 mb-6">
          ...
          <span className="text-[10px] text-muted-foreground font-medium ml-1">{step}/3</span>
        </div>
        ...
        <AnimatePresence mode="wait">
          ...
          {/* Step 2: Thông tin doanh nghiệp + Dịch vụ */}
          {step === 2 && (
          ...
          {/* Step 3: Xác nhận */}
          {step === 3 && (
            <motion.div key="s3" className="space-y-4" initial={{ opacity: 0, x: 20 }} animate={{ opacity: 1, x: 0 }} exit={{ opacity: 0, x: -20 }}>
              <div className="bg-muted/40 rounded-2xl p-4 space-y-3">
                <p className="text-[12px] font-bold text-foreground">Tóm tắt đăng ký</p>
                <div className="text-[11px] text-muted-foreground space-y-1.5">
                  <p className="font-semibold text-foreground text-[13px]">👤 Người đăng ký</p>
                ...
                {form.companyName && (
                  <div className="text-[11px] text-muted-foreground space-y-1.5 pt-2 border-t border-border/30">
                    <p className="font-semibold text-foreground text-[13px]">🏢 Doanh nghiệp</p>
                    <p>{form.companyName} · MST: {form.taxCode}</p>
                    <p>📍 {form.companyAddress}</p>
                  </div>
                )}
                {form.selectedServices.length > 0 && (
                  <div className="text-[11px] text-muted-foreground space-y-1.5 pt-2 border-t border-border/30">
                    <p className="font-semibold text-foreground text-[13px]">🔧 Dịch vụ đăng ký</p>
                    <div className="flex flex-wrap gap-1.5">
                      {form.selectedServices.map((id) => {
                        const svc = availableServices.find((s) => s.id === id);
                        return (
                          <span key={id} className="text-[10px] font-bold px-2 py-1 rounded-full bg-primary/10 text-primary">
                            {svc?.label}
                          </span>
                        );
                      })}
              ...
              <div>
                <label className="text-[12px] font-medium text-muted-foreground mb-1.5 block">Ghi chú bổ sung</label>
                <Textarea
        ...
        <div className="flex gap-3 mt-8">
          ...
          {step < 3 ? (
            <Button className="flex-1 touch-target font-bold rounded-2xl gradient-primary border-0 shadow-glow h-14 text-primary-foreground gap-2" onClick={() => setStep((s) => s + 1)}>
              Tiếp theo <ChevronRight size={16} />
            </Button>
          ) : (
            <Button className="flex-1 touch-target font-bold rounded-2xl gradient-primary border-0 shadow-glow h-14 text-primary-foreground gap-2" onClick={handleSubmit}>
