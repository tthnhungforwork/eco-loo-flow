...
import { useRole, ROLE_PROFILES, type RoleType } from "@/contexts/RoleContext";
import { Check, X, ArrowRightLeft, Plus } from "lucide-react";
import { motion, AnimatePresence } from "framer-motion";

const RoleSwitcherSheet = () => {
  const { currentRole, setRole, isSheetOpen, closeSheet } = useRole();
  const navigate = useNavigate();

  const handleSwitch = (id: RoleType) => {
    const profile = ROLE_PROFILES.find((r) => r.id === id);
    setRole(id);
    if (profile) {
    ...
    <AnimatePresence>
      {isSheetOpen && (
        <>
          ...
          >
            <div className="bg-card rounded-t-3xl shadow-elevated border-t border-border/50 overflow-hidden">
              {/* Handle */}
              ...
              <div className="px-4 py-3 space-y-1">
                {ROLE_PROFILES.map((profile, i) => {
                  const isActive = currentRole.id === profile.id;
                  return (
                    <motion.button
                      key={profile.id}
              ...
              <div className="px-4 py-3 safe-bottom">
                <button className="w-full flex items-center gap-3.5 px-3 py-3 rounded-2xl hover:bg-muted/50 active:bg-muted/80 transition-colors touch-target text-left">
                  ...
                  <div>
                    ...
                    <p className="text-[11px] text-muted-foreground mt-0.5">Trở thành Đối tác hoặc Doanh nghiệp</p>
                  </div>
                </button>
              </div>
            </div>
